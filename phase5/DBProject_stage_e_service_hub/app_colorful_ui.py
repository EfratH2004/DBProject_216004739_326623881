...
CARD_COLORS = [INFO, SUCCESS, ORANGE, PURPLE, TEAL, PINK, WARNING, PRIMARY, DANGER]
...
class ServiceHubApp(tk.Tk):
    ...
    def setup_styles(self):
        ...
        style.configure("Treeview", background=CARD, foreground=TEXT, fieldbackground=CARD,
                        borderwidth=0, rowheight=38, font=("Segoe UI", 10))
    ...
    def danger_button(self, parent, text, command, width=20):
        return tk.Button(parent, text=text, command=command, bg="#FFE4E6", fg=DANGER,
                         activebackground="#FECDD3", activeforeground=DANGER,
                         font=("Segoe UI", 10, "bold"), relief="flat", padx=12,
                         pady=10, width=width, cursor="hand2", bd=0)
    ...
    def icon_circle(self, parent, icon, color):
        circle = tk.Frame(parent, bg=color, width=58, height=58)
        circle.pack_propagate(False)
        tk.Label(circle, text=icon, bg=color, fg="white", font=("Segoe UI", 22, "bold")).pack(expand=True)
        return circle
    ...
    def action_card(self, parent, title, subtitle, command, accent=PRIMARY, icon="•", button_text="Open"):
        ...
        self.icon_circle(top, icon, accent).pack(side="left", padx=(0, 16))

        text_area = tk.Frame(top, bg=CARD)
        text_area.pack(side="left", fill="both", expand=True)

        tk.Label(text_area, text=title, bg=CARD, fg=accent,
                 font=("Segoe UI", 13, "bold")).pack(anchor="w")
        tk.Label(text_area, text=subtitle, bg=CARD, fg=SUBTEXT, font=("Segoe UI", 9),
                 wraplength=270, justify="left").pack(anchor="w", pady=(6, 14))
    ...
    def sidebar_layout(self, title, items):
        ...
        logo = tk.Frame(brand, bg="#7C3AED", width=54, height=54)
        logo.pack(side="left")
        ...
        for index, (label, command) in enumerate(items):
            ...
            active = "#3B2DB8" if not is_logout else "#7F1D1D"
    ...
    def show_manager_dashboard(self):
        body = self.sidebar_layout("Manager Dashboard", [
            ...
            ("📊  Manager Queries", self.show_old_open_requests_query),
            ("👥  Employees Table", self.show_employees_table),
            ("▶  Run Processes", self.run_workload_balance),
        ...
        cards = [
            ...
            ("Enter Employee Screens", "Enter employee screens and manage requests.", self.show_employee_dashboard, INFO, "🖥", "Open"),
            ("Logout", "Sign out from the system securely.", self.show_login, DANGER, "↪", "Logout"),
        ]
        ...
        for i, (title, sub, cmd, color, icon, btn_text) in enumerate(cards):
            ...
            c.grid(row=i // 3, column=i % 3, sticky="nsew", padx=10, pady=10)
