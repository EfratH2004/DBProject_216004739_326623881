...
class ServiceHubApp(tk.Tk):
    ...
    def action_card(self, parent, title, subtitle, command, accent=PRIMARY):
        box = tk.Frame(parent, bg=CARD, padx=22, pady=18,
                       highlightbackground=BORDER, highlightthickness=1)
        tk.Label(box, text=title, bg=CARD, fg=TEXT,
                 font=("Segoe UI", 13, "bold")).pack(anchor="w")
        tk.Label(box, text=subtitle, bg=CARD, fg=SUBTEXT, font=("Segoe UI", 9),
                 wraplength=245, justify="left").pack(anchor="w", pady=(6, 16))
        tk.Button(box, text="Open", command=command, bg=accent, fg="white",
                  activebackground=PRIMARY_DARK, activeforeground="white",
                  font=("Segoe UI", 10, "bold"), relief="flat",
                  padx=16, pady=8, cursor="hand2", bd=0).pack(anchor="w")
        return box
    ...
    def sidebar_layout(self, title, items):
        ...
        side = tk.Frame(shell, bg=SIDEBAR, width=245)
        side.pack(side="left", fill="y")
        side.pack_propagate(False)

        tk.Label(side, text="Service Hub", bg=SIDEBAR, fg="white",
                 font=("Segoe UI", 22, "bold")).pack(anchor="w", padx=24, pady=(28, 2))
        tk.Label(side, text="Management System", bg=SIDEBAR, fg="#94A3B8",
                 font=("Segoe UI", 10)).pack(anchor="w", padx=24, pady=(0, 28))
        ...
        for label, command in items:
            tk.Button(side, text=label, command=command, anchor="w",
                      ...
                      padx=22, pady=13, cursor="hand2", bd=0).pack(fill="x", padx=10, pady=3)
    ...
    def topbar_in(self, parent, title):
        bar = tk.Frame(parent, bg=CARD, height=76, highlightbackground=BORDER, highlightthickness=1)
        bar.pack(fill="x")
        bar.pack_propagate(False)
        tk.Label(bar, text=title, bg=CARD, fg=TEXT,
                 font=("Segoe UI", 22, "bold")).pack(side="left", padx=28)
        user_name = self.current_user.get("ename", "Guest") if self.current_user else "Guest"
        tk.Label(bar, text=f"● Online   {user_name}", bg="#F8FAFC", fg="#334155",
                 font=("Segoe UI", 10, "bold"), padx=18, pady=9).pack(side="right", padx=24)
    ...
    def show_login(self):
        ...
        left = tk.Frame(wrapper, bg=PRIMARY, width=470)
        left.pack(side="left", fill="y")
        left.pack_propagate(False)

        tk.Label(left, text="Service Hub", bg=PRIMARY, fg="white",
                 font=("Segoe UI", 34, "bold")).pack(anchor="w", padx=48, pady=(90, 8))
        tk.Label(left, text="A smart customer service\nmanagement system.",
                 bg=PRIMARY, fg="#E0E7FF", font=("Segoe UI", 16),
        ...
        for line in ["✓ Manage requests", "✓ Track products and transactions", "✓ Run manager reports"]:
            tk.Label(left, text=line, bg=PRIMARY, fg="white",
    ...
    def show_manager_dashboard(self):
        ...
        hero = self.card(body, padx=30, pady=24)
        hero.pack(fill="x", pady=(0, 24))
        tk.Label(hero, text="Revenue This Year", bg=CARD, fg=SUBTEXT,
                 font=("Segoe UI", 12, "bold")).pack(anchor="w")
        tk.Label(hero, text=f"${revenue:,.2f}", bg=CARD, fg=TEXT,
                 font=("Segoe UI", 34, "bold")).pack(anchor="w", pady=(6, 0))
        tk.Label(hero, text="Manager-only financial overview", bg=CARD, fg=SUBTEXT,
                 font=("Segoe UI", 10)).pack(anchor="w", pady=(2, 0))

        queries = tk.Frame(body, bg=BG)
        queries.pack(fill="x", pady=(0, 18))
        self.section_title(queries, "Manager Queries", "Run reports and view business insights")

        qgrid = tk.Frame(queries, bg=BG)
        qgrid.pack(fill="x")
        cards = [
            ("Open Requests", "Requests that have been open for more than 30 days.", self.show_old_open_requests_query),
            ("Product Revenue", "Sales count and total revenue for each product.", self.show_product_sales_query),
            ("Customer Spending", "Total amount spent by each customer.", self.show_customer_spending_query),
            ("Employees", "View all employees and workload information.", self.show_employees_table),
        ]
        for i, (title, sub, cmd) in enumerate(cards):
            c = self.action_card(qgrid, title, sub, cmd)
            c.grid(row=i // 2, column=i % 2, sticky="nsew", padx=10, pady=10)
        qgrid.columnconfigure(0, weight=1)
        qgrid.columnconfigure(1, weight=1)

        processes = tk.Frame(body, bg=BG)
        processes.pack(fill="x", pady=(8, 0))
        self.section_title(processes, "Processes", "Run database functions and procedures")

        pgrid = tk.Frame(processes, bg=BG)
        pgrid.pack(fill="x")
        process_cards = [
            ("Workload Balance", "Run the employee workload balancing process.", self.run_workload_balance),
            ("Supply & Demand", "Update low-stock product prices.", self.run_supply_demand),
            ("Discount Products", "Apply a discount to selected products.", self.show_discount_screen),
            ("Employee Area", "Enter the employee screens as manager.", self.show_employee_dashboard),
        ]
        for i, (title, sub, cmd) in enumerate(process_cards):
            c = self.action_card(pgrid, title, sub, cmd, accent=SUCCESS if i == 2 else PRIMARY)
            c.grid(row=i // 2, column=i % 2, sticky="nsew", padx=10, pady=10)
    ...
    def show_employee_dashboard(self):
        back_items = [("Manager Dashboard", self.show_manager_dashboard)] if self.current_user and self.current_user.get("role") == "manager" else []
        body = self.sidebar_layout("Employee Dashboard", [
            *back_items,
            ("Customers", lambda: self.show_crud_table("customers")),
            ("Requests", self.show_requests_screen),
            ("Transactions", self.show_transactions_screen),
            ("Products", lambda: self.show_crud_table("products")),
            ("Logout", self.show_login),
        ])

        welcome = self.card(body, padx=30, pady=24)
        welcome.pack(fill="x", pady=(0, 24))
        tk.Label(welcome, text=f"Welcome, {self.current_user.get('ename', '')}", bg=CARD, fg=TEXT,
                 font=("Segoe UI", 26, "bold")).pack(anchor="w")
        tk.Label(welcome, text="Choose a workspace to manage daily customer service operations.",
                 bg=CARD, fg=SUBTEXT, font=("Segoe UI", 11)).pack(anchor="w", pady=(6, 0))
        ...
        actions = [
            ("Customers", "View, search, add and update customers.", lambda: self.show_crud_table("customers")),
            ("Requests", "Manage your assigned service requests.", self.show_requests_screen),
            ("Transactions", "View your transactions with friendly names.", self.show_transactions_screen),
            ("Products", "Manage active products and inventory.", lambda: self.show_crud_table("products")),
        ]

        for i, (title, sub, cmd) in enumerate(actions):
            c = self.action_card(grid, title, sub, cmd)
            c.grid(row=i // 2, column=i % 2, sticky="nsew", padx=10, pady=10)
